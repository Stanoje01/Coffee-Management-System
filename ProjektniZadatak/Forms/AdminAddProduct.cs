...
namespace ProjektniZadatak
{
    public partial class AdminAddProduct : UserControl
    {
                ...
                if (connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        using (SqlCommand selectPID = new SqlCommand(selectProdID, connect))
                        {
                            ...
                            {
                                MessageBox.Show("Proizvod ID: " + adminAddProducts_id.Text.Trim() + " je već zauzet.", "Error Message"
                            ...
                            else
                            {
                                ...
                                string path = Path.Combine(@"C:\Users\Sasa\Desktop\C#\ProjektniZadatak\ProjektniZadatak\SlikeProizvoda\"
                                ...
                                using (SqlCommand cmd = new SqlCommand(insertData, connect))
                                {
                                    ...
                                    MessageBox.Show("Uspešno dodato!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    displayData();
                                }
                            }
                        }
        ...
        private void adminAddProducts_updateBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult check = MessageBox.Show("Da li ste sigurni da želite da ažurirate proizvod sa ID-em: " + adminAddProducts_id.Text.Trim() + "?"
                    ...
                    if (connect.State != ConnectionState.Open)
                    {
                        try
                        {
                            ...
                            using (SqlCommand updateD = new SqlCommand(updateData, connect))
                            {
                                ...
                                MessageBox.Show("Uspešno ažurirano!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                displayData();
        ...
        private void adminAddProducts_deleteBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                DialogResult check = MessageBox.Show("Da li ste sigurni da želite da izbrišete proizvod sa ID-em: " + adminAddProducts_id.Text.Trim() + "?"
                    , "Confirmation Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    ...
                    if (connect.State != ConnectionState.Open)
                    {
                        try
                        {
                            ...
                            using (SqlCommand updateD = new SqlCommand(updateData, connect))
                            {
                                ...
                                MessageBox.Show("Uspešno obrisan!", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                displayData();
